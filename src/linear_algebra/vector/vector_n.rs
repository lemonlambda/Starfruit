        ...
        fmt.write_str(
            (match self.vector_type {
                VectorType::Row => format!(
                    "[{}{}]",
                    self.storage[0..self.storage.len() - 1]
                        .iter()
                        .map(|n| format!("{} ", n))
                        .collect::<String>(),
                ...
                VectorType::Column => format!(
                    "┌{}┐\n{}└{}┘",
                    self.storage[0],
                    self.storage[1..(self.storage.len() - 1)]
                        .iter()
                        .map(|n| format!("│{}│\n", n))
                        .collect::<String>(),
                    self.storage[self.storage.len() - 1]
...
mod tests {
    ...
    fn operators() {
        ...
        assert_eq!(
            vec.clone() + 10,
            VectorN::new(vec![20, 30, 40, 50, 60], VectorType::Row)
        );
        assert_eq!(
            vec.clone() - 10,
            VectorN::new(vec![0, 10, 20, 30, 40], VectorType::Row)
        );
        assert_eq!(
            vec.clone() * 10,
            VectorN::new(vec![100, 200, 300, 400, 500], VectorType::Row)
        );
